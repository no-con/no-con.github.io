...
需要注意到是，对变量进行排序时，当两个(或更多变量)相同时，它们的次序值是通过对它们位置进行平均而得到的。比如，一组变量为 1, 3, 3, 4, 7, 将它们从小到大进行排序，3对应的次序应该是(2+3)/2 = 2.5;
...
$$
F 值越大，模型整体显著性水平越高，它是所估计回归的总显著性的一个度量，也是调整R-square的一个显著性检验。
