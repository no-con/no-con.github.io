...
2. LeNet
    ...
    LeNet5是一个 7 层的神经网络，包含 3 个**卷积层**，2 个**池化层**，1 个[**全连接层**](#compConnect)。其中所有卷积层的所有卷积核都为 5x5，步长 stride = 1，池化方法都为全局pooling，激活函数为 [Sigmoid](#sigmoid)，网络结构如下：
...
3. 卷积
    ...
    * 步长
        ...
        在进行卷积操作时，通常希望被卷积所得的图像分辨率与卷积前的图像分辨率相比逐渐减少，即图像被约减。步长方法通过改变卷积核在被卷积图像中移动步长的大小来跳过一些像素，进行卷积滤波。当stride = 1时，卷积核滑动跳过1个像素，这是最基本的单步滑动，也是标准的卷积模式。
